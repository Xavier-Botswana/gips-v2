...
export default function FirstRegistrationDetails({ isEdit, currentUser, applicantDetails }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      study_semester: currentUser?.semesterName || '',
      dob: formattedDate || '',
      ...
      study_year: currentUser?.year_of_study || '',
  ...
  const onSubmit = async (data) => {
    try {
      ...
      axios.patch(`/v1/applications/${currentUser.id}`, application).then(() => {
        navigate(PATH_DASHBOARD.admissions.firstTimeRegistrationlist);
      });
    } catch (error) {
      console.error(error);
    }
        ...
        <Grid item xs={12} md={16}>
          <Card sx={{ p: 3 }}>
            ...
            <Box sx={{ marginBottom: 2 }}>
              ...
              >
                {currentUser.registration_type === 'Returning' ? (
                  ''
                ) : (
                  <>
                    {' '}
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1.5} sx={{ mt: 3 }}>
              <Button
                ...
                to={PATH_DASHBOARD.admissions.firstTimeRegistrationlist}
                variant="outlined"
                size="large"
              >
                Discard
